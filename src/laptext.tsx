...
// Define the type for your symbol
type SymbolType = {
  x: number;
  y: number;
  angle: number;
...
const LapText = forwardRef<HTMLDivElement, {}>((props, ref) => {
  ...
  useEffect(() => {
    if (!pathRef.current) return;
    const newSymbols: SymbolType[] = [];
    const total_length = pathRef.current.getTotalLength();
    for (let i = 0; i < text.length; i++) {
      const position = (text_spacing * i + count) % total_length;
      const point = pathRef.current.getPointAtLength(position);
      const nextPoint = pathRef.current.getPointAtLength((position + 0.1) % total_length);
  ...
  }, [count, text, text_spacing]);

  return (
    <div className="svgWrapper" ref={ref}>
      <svg
        viewBox="0 0 920 610"
        width="100%"
        height="100%"
